...
export default function ExperimentClient() {
  ...
  return (
    <div className="min-h-screen">
      <main className="px-3 md:px-6 lg:px-8 py-4 md:py-6 max-w-2xl lg:max-w-4xl mx-auto">
        ...
        {experiment.date_completed && (
          <div className="text-sm md:text-base text-green-600 font-medium mb-4 md:mb-6">
        ...
        {!experiment.date_completed && (
          <Card className="p-4 md:p-6 mb-4 md:mb-6 bg-card/50 backdrop-blur-sm border-border/50">
            <div className="flex items-center justify-between">
              <div className="text-sm md:text-base font-medium text-foreground">Funding Deadline</div>
        ...
        {experiment.summary && (
          <Card className="p-4 md:p-6 mb-4 md:mb-6 bg-card/50 backdrop-blur-sm border-border/50">
            <h2 className="text-xl md:text-2xl font-semibold text-foreground mb-2 md:mb-3">About This Experiment</h2>
            <p className="text-muted-foreground md:text-base leading-relaxed whitespace-pre-wrap">
              {experiment.summary}
            </p>
          </Card>
        )}
        ...
        {experiment.date_completed && experiment.experiment_url && (
          <Card className="p-4 md:p-6 mb-4 md:mb-6 bg-muted/50 border-border/50">
          ...
          <>
            <Card className="mb-4 md:mb-6 bg-card border-border">
              <CardHeader className="px-4 md:px-6 pb-3 md:pb-4">
                <CardTitle className="text-lg md:text-xl font-semibold text-foreground">Funding Progress</CardTitle>
              </CardHeader>
              <CardContent className="px-4 md:px-6 pb-4 md:pb-6 pt-0 space-y-5 md:space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-6 text-left md:text-center">
                  <div>
                    <div className="text-2xl md:text-3xl font-bold text-secondary">
                      ${totalBet0USD.toLocaleString()}
                    </div>
                    <div className="text-muted-foreground text-xs md:text-sm">Amount {experiment.outcome_text0}</div>
                  </div>
                  <div>
                    <div className="text-2xl md:text-3xl font-bold text-secondary">
                      ${totalBet1USD.toLocaleString()}
                    </div>
                    <div className="text-muted-foreground text-xs md:text-sm">Amount {experiment.outcome_text1}</div>
                  </div>
                </div>
            ...
            <div className="relative mb-6 md:mb-8 rounded-2xl bg-gradient-to-br from-primary/35 via-transparent to-secondary/35 p-[1.5px] shadow-[0_20px_50px_-22px_rgba(124,58,237,0.6)]">
              ...
              <Card className="relative h-full rounded-[1.1rem] border-2 border-primary/40 bg-card/95 backdrop-blur">
                <CardHeader className="pb-2 md:pb-4">
                  <CardTitle className="text-lg md:text-xl font-semibold text-foreground">Fund and Bet</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 md:space-y-6">
                  {experimentState === 'open' && (
                    <>
                      ...
                      {currentStep === 'complete' ? (
                        ...
                        <form onSubmit={handleFunding} className="space-y-3 md:space-y-4">
                          <div>
                  ...
                  {isConnected && (
                    <Card className="mt-4 md:mt-6 p-3 md:p-4 bg-secondary/10 border-secondary/20">
                      <div>
                        <div className="text-xs md:text-sm text-secondary-foreground mb-1">Your Base USDC Balance</div>
                        <div className="text-xl md:text-2xl font-bold text-secondary">${userBalanceUSD.toLocaleString()}</div>
                        <div className="text-xs md:text-sm text-muted-foreground">Available for funding</div>
                      </div>
                    </Card>
                  )}

                  {/* Your Current Stake */}
                  <div className="mt-3 md:mt-4 rounded-lg border border-border/70 bg-card/90 p-3 md:p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3 md:gap-4">
                      <div>
                        <p className="text-[11px] md:text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Your Current Stake</p>
                        <p className="text-lg md:text-xl font-semibold text-foreground">${formatUsd(userTotalStakeUSD)}</p>
                        <p className="text-[11px] md:text-xs text-muted-foreground">Total funding + bets</p>
                      </div>
                    ...
                    {userTotalStakeUSD > 0 ? (
                      <div className="mt-3 md:mt-4 grid gap-x-6 gap-y-2 text-xs md:text-sm text-muted-foreground md:grid-cols-3">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <p className="text-[11px] md:text-xs uppercase tracking-wide text-primary/80">Locked until settlement</p>
                        </div>
                        <div className="space-y-0.5">
                          <p className="font-medium text-foreground">Bet {experiment.outcome_text1}: ${formatUsd(userBet1USD)}</p>
                          <p className="text-[11px] md:text-xs uppercase tracking-wide text-secondary-foreground/80">Locked until settlement</p>
                        </div>
                      </div>
                    ) : (
                      <p className="mt-3 md:mt-4 rounded-md bg-muted/30 px-3 py-2 text-sm md:text-base text-muted-foreground">
                  ...
                  {isConnected && (
                    <div className="mt-3 md:mt-4 bg-green-50 border border-green-200 rounded-lg p-3 md:p-4">
                      <div className="flex items-center gap-2 text-green-700">
                        <CheckCircle className="w-4 h-4 md:w-5 md:h-5" />
                        <span className="font-medium text-sm md:text-base">Connected: {address?.slice(0, 6)}...{address?.slice(-4)}</span>
                      </div>
